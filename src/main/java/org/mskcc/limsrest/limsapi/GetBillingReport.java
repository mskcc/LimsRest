       ...
       if(flowcell.size() > 0){
         ...
         if(possibleRun.size() > 0){
          ...
          if(!runList.contains(possibleRun.get(0))){
           ...
           String requestId = "";
           try{ requestId = req.getStringVal("RequestId", user); } catch(NullPointerException npe){}

           String labHead = "";
           try { labHead = req.getStringVal("LaboratoryHead", user);} catch(NullPointerException npe){}

           String investigator = "";
           try {investigator = req.getStringVal("Investigator", user); } catch(NullPointerException npe){}

           long dateRequested = 0;
           try { dateRequested = req.getDateVal("RequestStartDate", user); } catch(NullPointerException npe){}
           
           long dateRunReceived = 0;
           try { dateRunReceived = possibleRun.get(0).getDateVal("DateCreated", user); } catch(NullPointerException npe){}
