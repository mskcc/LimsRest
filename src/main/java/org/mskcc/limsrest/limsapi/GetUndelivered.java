 ...
 public Object execute(VeloxConnection conn){
    ...
    try {
        ...
        for (DataRecord undel: undelivered) {
            ...
            try{ deliveryDate = undel.getLongVal("DeliveryDate", user); } catch (NullPointerException npe){}
    ...
    for(DataRecord request : undeliveredSet) {
        try {
            ...
            try { rs.setSampleNumber(((Short) requestFields.get("SampleNumber")).intValue()); } catch (Exception e) {
                log.info(e.getMessage());
            }
            undelivered.add(rs);
