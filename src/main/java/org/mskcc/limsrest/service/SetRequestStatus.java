            ...
            for (DataRecord r : requestList) {
                ...
                while (fringe.size() > 0) {
                    ...
                    if (childrenSamples.length == 0) {
                        ...
                        status = "";
                        try {
                            childReq = child.getStringVal("RequestId", user);
                        } catch (Exception e) {
                        }
                        try {
                            status = current.getStringVal("ExemplarSampleStatus", user);
                        ...
                        for (DataRecord child : childrenSamples) {
                            try {
                                status = child.getStringVal("ExemplarSampleStatus", user);
                            } catch (Exception e) {
                            }
                        ...
                        status = "";
                        try {
                            childReq = leastProcessedChild.getStringVal("RequestId", user);
                        } catch (Exception e) {
                        }
                        try {
                            status = leastProcessedChild.getStringVal("ExemplarSampleStatus", user);
                        } catch (Exception e) {
                        }
                        if (!status2Count.containsKey(status)) {
                            status2Count.put(status, 0);
