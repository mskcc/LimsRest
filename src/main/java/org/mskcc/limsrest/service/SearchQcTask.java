...
public class SearchQcTask {
    private static Log log = LogFactory.getLog(SearchQcTask.class);
    ...
    // Pattern for recent run: text with underscore and number (e.g., BONO_0033, FAUCI_0166)
    private static final Pattern RECENT_RUN_PATTERN = Pattern.compile("^[A-Za-z]+_[0-9]+.*$");

    public SearchQcTask(String search, int limit, int offset, ConnectionLIMS conn) {
        this.search = search;
        this.limit = limit;
        this.offset = offset;
        this.conn = conn;
    ...
    public SearchQcResponse execute() {
        ...
        try {
            ...
            // Execute appropriate search based on pattern
            switch (searchType) {
                case PI_NAME:
